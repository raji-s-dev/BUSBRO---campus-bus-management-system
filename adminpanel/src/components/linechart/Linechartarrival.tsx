...
interface LineChartProps {
  title: string;
  label: string;
  dataPoints: (number | null)[]; // allow null
  target?: number;
}

// Labels must be strings to avoid Chart.js "shifting"
const labels: string[] = Array.from({ length: 30 }, (_, i) => String(i + 1));


// ✅ Fixed range for Arrival chart
const range = { min: 7.5, max: 9.5, title: "Arrival Time (HH:mm)" };

const LineChartArrival: React.FC<LineChartProps> = ({
  title,
  label,
  dataPoints,
  target,
...
pointBorderColor: dataPoints.map((val) => {
  ...
  return "green";
    ...
    if (isNaN(current) || isNaN(next)) return "transparent"; // weekend gap
  ...
  const options: ChartOptions<"line"> = {
    ...
    plugins: {
      ...
      if (value === null || isNaN(value)) return `${context.dataset.label}: (no data)`;
    ...
    color: (ctx) => {
      ...
      // If the datapoint is null → weekend → red
      return dataPoints[index] === null ? "red" : "#666";
    },
     callback: (val, index) => labels[index], // use your labels array directly
  }
 },
    },
  };
...
export default React.memo(LineChartArrival);
